...
export function buildRoundPrompt(
  ...
  rules.push(
    opts.manual
      ? `本轮为手动触发（人在场显式指令）：即使 ${declaration.dir}/PAUSED 或根目录 loop-pause-all 存在，也不据此收尾——照常执行完整流程（chandao-sync → 选品/收养 → 执行 → STATE/ledger 收尾）；暂停旗继续压制后续自动心跳，本轮不删不改旗；工作区 loop-constraints.md / SKILL.md 中「发现 PAUSED 立即收尾」条款与本条冲突时，以本条（宿主注入）为准。`
