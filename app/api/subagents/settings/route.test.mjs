...
test("settings route defaults ON (fork continuity) and persists both switch states", async () => {
  let response = await GET();
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { enabled: true, maxConcurrent: 10 }); // fork default ON
...
test("settings route validates and persists concurrency", async () => {
  ...
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { enabled: true, maxConcurrent: 2 }); // fork default ON
