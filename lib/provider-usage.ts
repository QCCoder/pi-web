...
export async function queryProviderUsage(providerId: ProviderUsageId): Promise<ProviderUsageResult> {
  ...
  try {
    const payload = await fetchProviderUsageJson(providerId, resolved.auth);
...
// fetchImpl is injectable so tests can stub the network (timeouts, HTTP
// errors, oversized payloads) without touching the outside world.
