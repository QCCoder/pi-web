...
/**
 * Fork adaptation: children keep the `pi-subagent-<uuid>` id prefix the
 * community-package era established, so `lib/subagent-child.ts` detection,
 * sidebar hiding and the locate/open flow work unchanged (upstream children
 * use bare SDK uuids). Passing the id via NewSessionOptions makes the
 * in-process child carry it from the very first append to the session file.
 */
function subagentChildSessionId(): string {
  return `pi-subagent-${randomUUID()}`;
}
...
/** pi's agent loop records provider failures as an assistant message with `stopReason: "error"` and resolves `prompt()` normally; surface that as a failed run. (upstream 9d282da) */
  ...
  async function start(request: StartSubagentRequest): Promise<SubagentExecution> {
    ...
    try {
      ...
      const extensionToolNames = profile.loadExtensions
        ...
        : [];
      // Fork adaptation: no PowerShell tool-name remapping (upstream's
      // resolveShellTools maps bash→powershell on Windows only; this fork
      // ships darwin/linux/docker surfaces).
      const activeTools = withSubagentExtensionTools(profile.tools, extensionToolNames);

      const sessionManager = isolatedWorktree
        ? SessionManager.create(childCwd, undefined, { id: subagentChildSessionId(), parentSession: parent.sessionFile })
        : SessionManager.create(parent.cwd, undefined, { id: subagentChildSessionId(), parentSession: parent.sessionFile });
