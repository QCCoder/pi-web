...
> 这是 orchestrator 在终态内联执行的归档规范（不另开子代理——learn 既不是 maker 也不是 checker，不需要隔离）。到终态时（final-verify 解析后、或早期泊车；round 0 idle 除外）按本文件执行两件事：写瘦审计 + 可选写知识库笔记。

## ① 瘦审计日志：`loops/dev-loop/LEARN.jsonl`
append 一条 JSON（一行，不自己重判，字段从 SPEC/VERDICT 抄）：
```json
...
```
- 一 run 一条。不写长 humanDecision——经验价值走②进知识库，不堆在 JSONL 里。
- 这是机器可读的审计轨迹（将来若复活校准数学的数据源），不是给人读的叙事。

## ② 知识库学习笔记（仅当通过泛化测试）
只有**可泛化的流程/判定级规则**才进知识库。代码指针级事实（类名/方法名/具体 KEY）不进——它们会随代码腐烂，对 kb_search 是噪音，归宿是工作项自己的 SPEC/PLAN/IMPLEMENTATION.md（在 git 里）。
...
- ❌ 不通过：`<某 ServiceImpl> 硬编码 OR 连接`（删掉具体名就什么都不是）
- 没通过的经验：只留①审计，不写②笔记。
...
- 一 run 最多一篇笔记（多条规则合并成一篇）。
- 模块级特定陷阱（费用状态机、敏感清单）归 `standards/dev-loop-modules.md`（人手维护 + loop append）；可泛化规则归 `learnings/`（loop 写）。两件产物不重叠。
