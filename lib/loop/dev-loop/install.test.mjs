...
test("createDevLoopDefinition ships a generic, autonomy-free definition the engine can read", async () => {
  ...
  // learner.md: learn ② (generalizability test + KB note formatting) — mechanical.
  const learner = await readFile(join(directory, "agents", "learner.md"), "utf8");
  assert.match(learner, /name: learner/);
  assert.match(learner, /泛化测试/);
  assert.match(learner, /learnings/);
  assert.match(learner, /author: loop/);
  assert.match(learner, /autoManaged: true/);
  assert.match(learner, /永不编辑已有笔记/);  // anti-pollution
  assert.match(learner, /written=<learnings/); // return protocol
  // LEARN.md as a top-level spec is gone — learn ② lives in the learner agent;
  // the orchestrator only does the audit line inline.
  await assert.rejects(() => readFile(join(directory, "LEARN.md"), "utf8"));
