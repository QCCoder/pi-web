...
test("createDevLoopDefinition ships a generic, autonomy-free definition the engine can read", async () => {
  ...
  assert.match(designer, /无论 planner 的 needsDesign/);  // Finding 2: trace overrides planner pre-judgement → must persist DESIGN.md
  ...
  assert.doesNotMatch(developer, /gate7/);  // Finding 1: no stale gate7
