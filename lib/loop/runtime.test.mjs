...
test("a heartbeat during a long round writes a progress snapshot and does not overtake the terminal one", async () => {
  const { path, runtime } = await fixture({
    startRound: async (_def, _run, onSessionReady, onProgress) => {
      if (onSessionReady) await onSessionReady("session-hb");
      // Simulate mid-round activity the real backend surfaces as a heartbeat.
      if (onProgress) onProgress({ detail: "tool: bash" });
      return { output: "done", verdict: "ok" };
    },
  });
  const receipt = await runtime.trigger({ workspaceId: "ws", loopId: "check", source: "manual", eventId: "event-hb" });
  const complete = await waitFor(runtime, receipt.runId, "succeeded");
  assert.equal(complete.status, "succeeded");
  // The terminal snapshot must not carry a stale progress hint.
  assert.equal(complete.progress, undefined);
  const lines = (await readFile(join(path, "loops", "check", "RUNS.jsonl"), "utf8"))
    .trim().split("\n").map((line) => JSON.parse(line));
  const heartbeat = lines.find((line) => line.progress === "tool: bash");
  assert.ok(heartbeat, "a heartbeat snapshot carrying progress was written mid-round");
  assert.equal(heartbeat.status, "running");
  // The fire-and-forget heartbeat is serialized before the terminal snapshot,
  // so the last line is the terminal state (not a stale heartbeat).
  assert.equal(lines[lines.length - 1].status, "succeeded");
  assert.equal(lines[lines.length - 1].progress, undefined);
});

test("reapOrphanedGates fails a gate run whose orchestrator file no longer exists", async () => {
  const { runtime } = await fixture({ gateOnStart: true });
  const receipt = await runtime.trigger({ workspaceId: "ws", loopId: "check", source: "manual", eventId: "event-reap" });
  await waitFor(runtime, receipt.runId, "waiting_for_gate");
  // The mock's sessionId ("session-1") has no real .jsonl on disk, so
  // resolveSessionPath returns null -> the gate is unrecoverable -> reaped.
  const reaped = await runtime.reapOrphanedGates();
  assert.ok(reaped >= 1, "at least one orphaned gate was reaped");
  const after = await runtime.getRun("ws", receipt.runId);
  assert.equal(after.status, "failed");
  assert.match(after.error, /expired/);
});

test("reapOrphanedGates leaves a gate run with no sessionId untouched", async () => {
  const { runtime } = await fixture({
    startRound: async () => ({ output: "paused", gateRequest: "need human" }),
  });
  const receipt = await runtime.trigger({ workspaceId: "ws", loopId: "check", source: "manual", eventId: "event-nosid" });
  await waitFor(runtime, receipt.runId, "waiting_for_gate");
  await runtime.reapOrphanedGates();
  const after = await runtime.getRun("ws", receipt.runId);
  assert.equal(after.status, "waiting_for_gate");
});

test("findBySessionId resolves the latest run of an orchestrator session", async () => {
  const { runtime } = await fixture();
  const receipt = await runtime.trigger({ workspaceId: "ws", loopId: "check", source: "manual", eventId: "event-find" });
  await waitFor(runtime, receipt.runId, "succeeded");

  const found = await runtime.findBySessionId("session-1");
  assert.equal(found?.workspaceId, "ws");
  assert.equal(found?.run.id, receipt.runId);
  assert.equal(found?.run.status, "succeeded");

  // Unknown session ids resolve to undefined (the host probe relies on this to
  // distinguish interactive/child sessions from orchestrators).
  assert.equal(await runtime.findBySessionId("no-such-session"), undefined);
});
