 ...
 *  shape startRound/abortRound behavior and observe calls. v3: runs are single
 *  selection rounds — no resumeRound/gate surface anymore. */
async function fixture(options = {}) {
  ...
  const execution = {
    startRound: options.startRound ?? (async (_def, run, onSessionReady) => {
      ...
      return options.seed
        ? { output: "picked", verdict: "LOOP_SEED: REQ-0001", seed: options.seed }
        : { output: "idle", verdict: "idle" };
    }),
...
test("trigger runs a single selection round and succeeds", async () => {
  ...
  assert.equal(complete.verdict, "idle");
  assert.deepEqual(calls.find((call) => call[0] === "startRound")?.slice(0, 1), ["startRound"]);
  assert.equal(calls.some((call) => call[0] === "resumeRound"), false);
});

test("a seeded round records seededSessionId on the terminal snapshot", async () => {
  const { runtime } = await fixture({
    seed: { key: "REQ-0001", sessionId: "exec-session-1" },
  });
  const receipt = await runtime.trigger({ workspaceId: "ws", loopId: "check", source: "manual", eventId: "event-seed" });
  const complete = await waitFor(runtime, receipt.runId, "succeeded");
  assert.equal(complete.seededSessionId, "exec-session-1");
});
...
test("abortRun during in-flight startRound wins (aborted guard prevents double-fail)", async () => {
  ...
  // startRound was attempted exactly once.
...
test("evidence log records the lifecycle without retired statuses", async () => {
  ...
  // No retired statuses should be written by the v3 engine.
  assert.doesNotMatch(evidence, /waiting_for_gate/);
