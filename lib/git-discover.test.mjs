...
test("`.pi` is pruned: the in-project pi data home never surfaces as nested repos", async () => {
  const { discoverRepoAndWorktreeRoots, discoverReposAndScattered } = await loadSubject();
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pi-disc-dotpi-"));
  try {
    const main = path.join(tmp, "main");
    initRepo(main);
    // Simulate the relocated pi data home (<project>/.pi with workspaces/
    // agent/ inside, each workspace being a git repo). Walking from the
    // project root must prune `.pi` entirely — the 19G data home must never
    // show up in the Changes panel or the file index.
    const wsA = path.join(main, ".pi", "workspaces", "workspace-a");
    initRepo(wsA);
    const wsB = path.join(main, ".pi", "agent", "sessions");
    initRepo(wsB);

    const { repoRoots } = discoverRepoAndWorktreeRoots(main);
    assert.deepEqual(repoRoots, [main]);

    const { scatteredFiles } = discoverReposAndScattered(main);
    assert.deepEqual(scatteredFiles, []);
  } finally {
    rmrf(tmp);
  }
