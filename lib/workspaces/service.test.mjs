...
test("effectiveCapabilities returns the manifest's cached capabilities when present", () => {
  const manifest = makeManifest({ capabilities: ["sessions", "explorer", "knowledge"] });
  assert.deepEqual(effectiveCapabilities(manifest), ["sessions", "explorer", "knowledge"]);
});

test("effectiveCapabilities falls back to [sessions, explorer] when capabilities are absent", () => {
  const manifest = makeManifest();
  assert.deepEqual(effectiveCapabilities(manifest), ["sessions", "explorer"]);
});

test("buildWorkspaceExtensions mounts modules by capability and ignores retired channel capabilities", async () => {
  const { buildWorkspaceExtensions } = await import("./extensions.ts");
  // A legacy manifest still carrying retired channel capabilities must not
  // mount anything for them (the modules are deleted) and must not throw.
  const manifest = makeManifest({
    capabilities: ["sessions", "explorer", "work-items", "knowledge", "feishu-transport", "feishu-channel", "wecom-channel"],
  });
  ...
  assert.ok(!names.some((n) => n.includes("feishu") || n.includes("wecom")), "retired channel modules must not mount");
});
