...
/** Teardown rm with retry: `git init`'s fs writes can land slightly after the
 *  process exits, and a plain recursive rm then dies with ENOTEMPTY on `.git`.
 *  A short retry window makes the fixture teardown robust to that race. */
    ...
    try {
      ...
      if (i === attempts - 1) {
        try {
          ...
          console.error("rmWithRetry giving up on", target, "leftover:", readdirSync(target, { recursive: true }).slice(0, 10));
        } catch { /* gone after all */ }
        throw error;
      }
...
test("retired capability values throw on manifest read (no read-path strip)", () => {
  const base = {
    schema_version: 1,
    id: "01KYPYG0HBH7WN4RK71XHZ7WTN",
    slug: "fixture",
  ...
  // The one-shot retirement cut is complete: a manifest still carrying a retired
  // value is config-invalid (the value is not in ALL_WORKSPACE_CAPABILITIES, so
  // parseCapabilities rejects it — the same as any unknown capability).
  for (const retired of ["overview", "loop", "requirement-sources"]) {
    assert.throws(
      () => parseWorkspaceManifest({ ...base, capabilities: ["sessions", "explorer", retired] }),
      WorkspaceValidationError,
    );
  }
  assert.throws(
...
test("a v1 on-disk index is rejected (strict v2)", async (t) => {
  const root = await fixture(t);
  await mkdir(join(root, ".pi"), { recursive: true });
  await writeFile(
    join(root, ".pi", "workspace.yaml"),
    stringify({ schema_version: 1, workspaces: [] }),
    "utf8",
  );
  await assert.rejects(
    discoverWorkspaces(root),
    (error) => error instanceof WorkspaceValidationError
      && /Unsupported global Workspace index schema: 1/.test(error.message),
  );
});
...
test("auto-scan sync: 消失自动停用、重现自动恢复、钉住 alias 不被扫描改写", async (t) => {
  ...
  // 磁盘上消失 → 自动停用（保留条目）
  await rmWithRetry(join(created.path, "proj-a"));
