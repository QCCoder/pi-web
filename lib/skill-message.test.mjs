...
test("reduces a truncated wrapper copy to the reconstructed command", () => {
  // What the session index stored before skill-aware truncation: the 400-char
  // clip lands inside the SKILL.md body — no closing tag, no arguments.
  const truncated = expanded.slice(0, 160);

  assert.equal(parseSkillMessage(truncated), null);
  assert.equal(skillMessageTitle(truncated), "/skill:grilling");

  // A wrapper that still carries its closing tag never hits the fallback,
  // even when the full parse rejects it (e.g. missing separator line).
  const closedButMalformed = `<skill name="grilling" location="/a/SKILL.md">\nuser text\n</skill>\narg`;
  assert.equal(skillMessageTitle(closedButMalformed), closedButMalformed);
});
