...
test("sign-in obtains a token via POST /tokens", async () => {
  ...
  const fetchImpl = mockFetch([
    ...
    { match: /\/api\.php\/v1\/products\/2\/bugs/, json: { bugs: [{ id: 50, title: "Login blank" }] } },
...
test("listAssigned merges bugs and tasks", async () => {
  const fetchImpl = mockFetch([
    ...
    { match: /products\/2\/bugs/, json: { bugs: [{ id: 50, title: "B1" }, { id: 51, title: "B2" }] } },
    { match: /executions\/3\/tasks/, json: { tasks: [{ id: 3, name: "T1" }] } },
  ]);
