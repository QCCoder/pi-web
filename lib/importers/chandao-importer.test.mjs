...
test("sign-in obtains a token via POST /tokens", async () => {
  ...
  const fetchImpl = mockFetch([
    ...
    { match: /\/api\.php\/v1\/products\/2\/bugs/, json: { bugs: [{ id: 50, title: "Login blank", status: "active" }] } },
...
test("listAssigned merges bugs and tasks", async () => {
  const fetchImpl = mockFetch([
    ...
    { match: /products\/2\/bugs/, json: { bugs: [{ id: 50, title: "B1", status: "active" }, { id: 51, title: "B2", status: "active" }] } },
    { match: /executions\/3\/tasks/, json: { tasks: [{ id: 3, name: "T1", status: "wait" }] } },
  ]);
...
test("listAssigned filters to ready statuses (bug active, task wait)", async () => {
  const fetchImpl = mockFetch([
    { match: /\/tokens$/, method: "POST", status: 201, json: { token: "t" } },
    {
    ...
    },
    {
      ...
      json: { tasks: [
        { id: 10, name: "wait", status: "wait" },
        { id: 11, name: "doing", status: "doing" },
        { id: 12, name: "done", status: "done" },
        { id: 13, name: "closed", status: "closed" },
      ] },
    },
  ]);
  const importer = new ChandaoImporter(baseConfig(), { fetchImpl });
