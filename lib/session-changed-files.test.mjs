...
test("apply_patch calls record Add/Update targets but not deletes (upstream e70c367 语义)", () => {
  const patch = [
    "*** Begin Patch",
    "*** Add File: new.ts",
    "+brand new",
  ...
  // 派生按"最近触前"排序: patch 里 Update 在 Add 之后记录。
  assert.deepEqual(deriveSessionChangedFiles(messages, { cwd: "/w" }), [
    { filePath: "/w/src/old.ts", kind: "edit", count: 1 },
    { filePath: "/w/new.ts", kind: "write", count: 1 },
  ]);
});

test("apply_patch name variants match and non-patch input records nothing", () => {
  const messages = [assistant([
    ["pi.apply_patch", { input: "*** Update File: a.ts\n-x\n+y" }],
    ["apply_patch_v2", { command: "not a patch document" }],
  ])];
  assert.deepEqual(deriveSessionChangedFiles(messages), [
    { filePath: "a.ts", kind: "edit", count: 1 },
  ]);
});
