...
test("default scope 'user' does NOT load project agents, falls back to built-in general", async () => {
  ...
  await withEnv({ [ENV_AGENT_DIR]: agentHome }, async () => {
    const { agents, projectAgentsDir } = discoverAgents(workspace, "user");
    // Project agent must NOT load under the safe default scope.
    assert.ok(!agents.find((a) => a.name === "researcher"), "project agent must not load under scope:user");
    assert.equal(agents.length, 1);
    assert.equal(agents[0].name, "general");
    // projectAgentsDir is still reported so the UI can offer to enable project agents.
    assert.ok(projectAgentsDir, "projectAgentsDir should be reported even when not loaded");
  });
...
test("scope 'both' discovers project agents and parses frontmatter + body", async () => {
  const workspace = await mkdtemp(join(tmpdir(), "pi-sub-ws-"));
  const agentHome = await mkdtemp(join(tmpdir(), "pi-sub-home-"));
  await makeAgentFile(join(workspace, ".pi", "agents"), "researcher", {
    description: "Deep research specialist",
  ...
  await withEnv({ [ENV_AGENT_DIR]: agentHome }, async () => {
    const { agents } = discoverAgents(workspace, "both");
...
test("scope 'project' loads only project agents", async () => {
  const workspace = await mkdtemp(join(tmpdir(), "pi-sub-ws-"));
  const agentHome = await mkdtemp(join(tmpdir(), "pi-sub-home-"));
  await makeAgentFile(join(agentHome, "agents"), "global-only", { body: "user level" });
  await makeAgentFile(join(workspace, ".pi", "agents"), "local-only", { body: "project level" });

  await withEnv({ [ENV_AGENT_DIR]: agentHome }, async () => {
    const { agents } = discoverAgents(workspace, "project");
    assert.ok(agents.find((a) => a.name === "local-only"));
    assert.ok(!agents.find((a) => a.name === "global-only"), "user agents must not load under scope:project");
  });
...
test("project agents override user agents with the same name (scope 'both')", async () => {
  const workspace = await mkdtemp(join(tmpdir(), "pi-sub-ws-"));
  const agentHome = await mkdtemp(join(tmpdir(), "pi-sub-home-"));
...
test("returns built-in general agent when nothing is defined", async () => {
  const workspace = await mkdtemp(join(tmpdir(), "pi-sub-ws-"));
  const agentHome = await mkdtemp(join(tmpdir(), "pi-sub-home-"));
  await withEnv({ [ENV_AGENT_DIR]: agentHome }, async () => {
    const { agents } = discoverAgents(workspace, "both");
