...
test("sidecarSpawnEnv translates PI_LOOP_URL into the daemon's bind address", () => {
  // PI_LOOP_URL-only configuration: the spawned daemon must listen where the
  // web client will probe, not on the default 30142.
  const env = sidecarSpawnEnv("http://127.0.0.1:33198", {});
  assert.equal(env.PI_LOOP_HOST, "127.0.0.1");
  assert.equal(env.PI_LOOP_PORT, "33198");
});

test("sidecarSpawnEnv keeps explicit PI_LOOP_HOST/PI_LOOP_PORT over the URL", () => {
  const env = sidecarSpawnEnv("http://127.0.0.1:33198", { PI_LOOP_PORT: "45555", PI_LOOP_HOST: "localhost" });
  assert.equal(env.PI_LOOP_PORT, "45555");
  assert.equal(env.PI_LOOP_HOST, "localhost");
});

test("sidecarSpawnEnv unbrackets IPv6 loopback hostnames", () => {
  const env = sidecarSpawnEnv("http://[::1]:33198", {});
  assert.equal(env.PI_LOOP_HOST, "::1");
});
