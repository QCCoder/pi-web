...
test("reads back whatever string the persistence layer holds", () => {
  const { fakeWindow } = installWindow();
  fakeWindow.localStorage.setItem("pi-thinking-expanded", "false");
  assert.equal(isThinkingExpandedByDefault(), false);
  fakeWindow.localStorage.setItem("pi-thinking-expanded", "true");
  assert.equal(isThinkingExpandedByDefault(), true);
});
