...
test("seeded path resolves a daemon-created session before its .jsonl exists (no 404 scan)", async () => {
  // Regression: a session created in the daemon does not write its .jsonl until
  // the first append. /api/agent/new seeds the id→path cache with the file path
  // so GET /api/sessions/:id resolves without a disk scan; the scan would miss
  // the not-yet-written file, 404, and poison the 30s session-list cache (the
  // "new session reply invisible until you switch away and back" bug).
  const sessionId = `seed-test-${Date.now()}`;
  const notYetWritten = join(tmpdir(), "pi-web-seed-test", "not-yet-written.jsonl");
  cacheSessionPath(sessionId, notYetWritten);
  try {
    assert.equal(await resolveSessionPath(sessionId), notYetWritten);
  } finally {
    invalidateSessionPathCache(sessionId);
  }
});
