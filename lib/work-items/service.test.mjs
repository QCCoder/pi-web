...
test("findWorkItemByConversation resolves the item a session linked to", async (t) => {
  const { workspace } = await fixture(t);
  await createWorkItem(workspace.id, {
    type: "requirement",
    title: "Unrelated",
    originalDescription: "Not linked.",
  });
  const linked = await createWorkItem(workspace.id, {
    type: "requirement",
    title: "Menu rendering",
    originalDescription: "Render the menu.",
    conversationId: "loop-session-42",
  });

  const found = await findWorkItemByConversation(workspace.path, "loop-session-42");
  assert.equal(found?.key, linked.item.key);
  assert.equal(found?.title, "Menu rendering");

  // unknown session id -> undefined
  assert.equal(await findWorkItemByConversation(workspace.path, "nope"), undefined);
  // missing work-items directory -> undefined (no throw)
  assert.equal(await findWorkItemByConversation(join(workspace.path, "does-not-exist"), "loop-session-42"), undefined);
});
...
test("external source ref is stamped at creation and round-trips", async (t) => {
  ...
  // listed items carry external (used by the importer dedup index)
