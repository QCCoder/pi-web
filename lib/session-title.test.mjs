...
test("removes expanded skill instructions before generating a title", () => {
  const expandedSkill = `<skill name="grilling" location="/tmp/grilling/SKILL.md">
References are relative to /tmp/grilling.
...
设计新的会话标题`;
  const messages = [
    { role: "user", content: [{ type: "text", text: expandedSkill }], timestamp: 1 },
  ];

  const sanitized = sanitizeTitleMessages(messages);

  assert.equal(sanitized[0].content[0].text, "设计新的会话标题");
  assert.equal(messages[0].content[0].text, expandedSkill);
});
