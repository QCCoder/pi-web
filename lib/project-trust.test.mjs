...
test("all project resource loaders and reloads enforce project trust", async () => {
  const rpcSource = await readFile(new URL("./daemon/rpc-manager.ts", import.meta.url), "utf8");
...
test("the trust API invalidates cached models and restricted runtimes", async () => {
  ...
  // Trust changes now go through the session daemon (C2): busy-check + cwd
  // reload are daemon routes, not local registry calls.
  assert.match(source, /client\.busyForCwd\(result\.cwd\)/);
  assert.match(source, /client\.reloadCwdSessions\(result\.cwd\)/);
