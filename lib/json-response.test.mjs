...
test("gzips large payloads and round-trips through gunzip (local addition)", async () => {
  const response = jsonResponse(
    new Request("http://localhost/test", { headers: { "Accept-Encoding": "gzip" } }),
    { value: "large payload ".repeat(200) },
  );
  ...
  assert.equal(JSON.parse(raw).value, "large payload ".repeat(200));
});

test("preserves caller headers such as ETag alongside compression (local addition)", async () => {
  const response = jsonResponse(
    new Request("http://localhost/test", { headers: { "Accept-Encoding": "gzip" } }),
    { value: "etagged payload ".repeat(200) },
    { headers: { ETag: '"1024-1234567890.5"' } },
  );

  assert.equal(response.headers.get("Content-Encoding"), "gzip");
  assert.equal(response.headers.get("ETag"), '"1024-1234567890.5"');
