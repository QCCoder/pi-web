...
function makeWorkspace(t, { withItem = true } = {}) {
  ...
  const dir = join(root, ".pi", "loops", "dev-loop");
  ...
  if (withItem) {
    const itemDir = join(root, ".pi", "work-items", "requirements", "REQ-0001-loop-manage-test");
...
test("writeLoopFile: mtime 冲突 → 409 带当前内容；省略 baseMtime = 覆盖", async (t) => {
  const root = makeWorkspace(t);
  const path = join(root, ".pi", "loops", "dev-loop", "chandao.md");
...
test("writeLoopFile: body 保存后 frontmatter 字节不变", async (t) => {
  const root = makeWorkspace(t);
  const path = join(root, ".pi", "loops", "dev-loop", "LOOP.md");
...
test("createLoop: 脚手架 + 参数校验", async (t) => {
  ...
  assert.ok(existsSync(join(root, ".pi", "loops", "ops-loop", "LOOP.md")));
  assert.ok(existsSync(join(root, ".pi", "loops", "ops-loop", ".lastrun"))); // initLoop 写 .lastrun=now
  assert.ok(existsSync(join(root, ".pi", "loop", "constraints.md"))); // 根宪法缺失时由 init 复制模板
  assert.ok(existsSync(join(root, ".pi", "skills", "ops-loop", "SKILL.md"))); // SKILL 骨架
...
test("deleteLoop: 锁活 409 / 绑定项 409+确认放行 / SKILL 与宪法不删", async (t) => {
  ...
  const skillPath = join(root, ".pi", "skills", "dev-loop", "SKILL.md");
  mkdirSync(dirname(skillPath), { recursive: true });
  writeFileSync(skillPath, "# skill\n");
  mkdirSync(join(root, ".pi", "loop"), { recursive: true });
  writeFileSync(join(root, ".pi", "loop", "constraints.md"), "# 宪法\n");
  ...
  assert.ok(existsSync(join(root, ".pi", "work-items", "requirements"))); // 工作项不动
  assert.ok(existsSync(skillPath)); // SKILL 不删（loop 目录外）
  assert.ok(existsSync(join(root, ".pi", "loop", "constraints.md"))); // 根宪法不删（可能被其它 loop 共享）
});
