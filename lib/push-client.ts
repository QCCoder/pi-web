...
export async function setupPushSubscription(locale: string): Promise<boolean> {
  ...
  const attempt = (async () => {
    try {
      ...
      // PwaRegistration registers the service worker in production builds
      // only, so resolve the existing registration instead of awaiting
      // `serviceWorker.ready` (which would never settle in dev / before the
      // first successful registration). setupPushSubscription is re-triggered
      // on later completions and settings re-registers, so a too-early caller
