...
const { createMapStore, isSliceCacheValid } = jiti("./create-map-store");
...
test("isSliceCacheValid: null prev is invalid (first read must compute)", () => {
  assert.equal(isSliceCacheValid(null, "a", 0), false);
});

test("isSliceCacheValid: same key + same version is valid (stability)", () => {
  assert.equal(isSliceCacheValid({ key: "a", version: 3 }, "a", 3), true);
});

test("isSliceCacheValid: same key, version changed is invalid", () => {
  assert.equal(isSliceCacheValid({ key: "a", version: 3 }, "a", 4), false);
});

test("isSliceCacheValid: key changed is invalid EVEN when version number collides", () => {
  // Regression: switching session a->b where both were loaded once (version 1).
  // The cache held a's slice; without a key check it would wrongly return a's
  // slice for b because keyVersion(a) === keyVersion(b) === 1.
  assert.equal(isSliceCacheValid({ key: "a", version: 1 }, "b", 1), false);
});

test("isSliceCacheValid: key and version both changed is invalid", () => {
  assert.equal(isSliceCacheValid({ key: "a", version: 1 }, "b", 2), false);
});
