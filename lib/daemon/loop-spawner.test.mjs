...
import { discoverKitLoops } from "../../packages/pi-loop/protocol.ts";
...
test("runKitRound happy path: creates one-shot session, names it, settles", async () => {
  ...
  // 合同本体在 LOOP.md；宪法三件/ledger 已退役（2026-09-04）——不再出现在轮 prompt
  assert.ok(fake.sentCommands.some((c) => c.type === "prompt" && c.message.includes("/ws/loops/dev-loop/STATE.md")));
  assert.ok(!fake.sentCommands.some((c) => c.type === "prompt" && c.message.includes("loop-ledger")));
...
function makeItemWorkspace() {
  ...
  const itemDir = join(ws, ".pi", "work-items", "requirements", "REQ-0001-test");
...
test("双 loop 独立性：a 轮在飞不阻塞 b（per-loop due/锁，跨 spawner 证明）", async () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(root, ".pi", "loops", "a", ".round.lock")), "a 轮仍在飞（锁持有中）");
    assert.ok(existsSync(join(root, ".pi", "loops", "b", ".lastrun")), "b 已写自己的 .lastrun");
    // 放行 a → 第一个 tick 完成，a 的锁释放；b 因 .lastrun 已写而不再补 fire
    resolveA();
    await tick1;
    assert.deepEqual(fired1, ["a"]);
    assert.ok(existsSync(join(root, ".pi", "loops", "a", ".lastrun")));
    assert.ok(!existsSync(join(root, ".pi", "loops", "a", ".round.lock")));
    assert.ok(!existsSync(join(root, ".pi", "loops", "b", ".round.lock")));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
