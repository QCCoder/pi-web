...
components/
  ...
  WorkspaceSidebar.tsx      middle-column content for module views (workbench/knowledge under a PanelHeader) + the home workspace-list panel; 工作台 desktop = sessions-only (`showFilesSection={false}` — the file tree lives in the right panel's 文件 tab), mobile = stacked collapsible 会话 + 文件 sections (drag-resizable share, default 40%, `pi-workbench-split:<wsId>`; [ 文件 | 改动(N) ] tabs + ⟳ manual refresh, `pi-workbench-sections:<wsId>` persistence)
