...
- **Attachment point**: `buildWorkspaceExtensions(manifest, path)` filters factories by `manifest.capabilities`. `lib/daemon/rpc-manager.ts` always attaches the built-in subagent extension globally (`createSubagentExtension` — see Subagent below), then — when the session's cwd is inside a workspace — appends `buildWorkspaceExtensions(...)` and filters skills to `manifest.skills`.
...
### Navigation: tree sidebar | main area（2026-09 树形侧栏改版，grill 共识）

Desktop is three vertical strips: **`[ProjectSidebar tree sidebar 200–560px drag-resizable/collapsible] [main area: ChatToolbar + SessionTabBar + CenterPage/Overview/chat] [file dock 42%, default-OPEN]`** — the 44px icon rail + middle-column-panel pair is RETIRED (W-中收敛后栏里只剩一个模块图标，为它维持双轨不值）；左侧合并为单一树形侧栏。**作用域即位置：树 = 工作区资源（会话/归档），底部 = 全局配置入口（设置/模型/插件/Skills，打开时可带工作区上下文）。** The top tab bar is SESSION-keyed（2026-09 会话 tab 化，docs/session-tabs-design.md）：每个 tab = 一个会话/新会话占位/工作区家 tab（总览），跨工作区混排；`SessionTabState` 纯逻辑（C1 点击分派/X1 邻居回落/U1 去重/URL 投影/R2 持久化恢复）在 `lib/session-tabs.ts`（+测试）。工作区从一级容器降级为跟随当前 tab 的环境上下文（`activeWorkspace = activeTab.workspace` 链不变）。交互：列表普通点击=当前 tab 原地变身（家 tab 上点击=开新 tab）、Cmd/中键=新开、fork=新 tab、关 tab=先左后右邻居回落（最后一个→首页）、会话删除/归档自动关 tab；＋=当前工作区开新占位 tab；**⊞ 工作区选择器退役（桌面——树常驻接管工作区导航；移动端保留，onPickWorkspace 可选 prop）**；tab 条全量存 `pi-session-tabs`，重载恢复（失效 id 静默跳过，URL 深链优先决定 active）；只挂载当前 tab 的 ChatWindow，tab 徽章靠 running/events 全局流。移动端同一 SessionTabBar（chips 行，首页落地页隐藏）。** **The file panel hosts the workbench file tree as a pinned, non-closable leading「文件」tab** (`FILES_TAB_ID` in `lib/tab-types.ts`, re-exported by `TabBar.tsx`; content = `FilesExplorerPanel`) ahead of the opened-file tabs — per-tab file-tab state rides the SESSION tab (F1)，the fixed top-right toggle still collapses the panel, closing the last file tab falls back to the「文件」tab instead of collapsing the panel, and the tree stays MOUNTED (hidden via display:none) across tab switches of the SAME workspace so expansion state survives. Clicking a file in the tree opens/activates its file tab (FileViewer's edit mode = the click-to-edit path). **2026-09 右坞 S1（docs/right-dock-design.md）：右面板升级为 tab 化扩展坞**——TabBar 的 `leadingTabs` 钉死模块组（文件 + Loops；工作项/知识库随 S3/S2 加入，门控见 design §5），模块 id 也存在 `activeFileTabId` 值域里（同一条 F1 机制）；模块体常驻挂载（display:none 隐藏，同工作区切会话 tab 状态存活，换工作区 key 重置）；面板 <420px 时模块 tab 图标化（`right-panel-container` 的 container query，模块 tab 永不滚走，文件 tab 照旧横滚）；Loops tab = `LoopsDockPanel`（loop 列表+状态/暂停恢复/停止/运行 + 点开内联 `LoopsConfig` 推导航）。

**项目树侧栏 = `components/ProjectSidebar.tsx`**：顶部「＋ 新建任务」= 上下文快速路径（有活动工作区→handleWorkspaceNewSession 开新占位 tab，首页→handleReturnHome 首页 composer）；**两处 composer 控制行都带工作区选择器**（共享 `WorkspaceSelector`，从 HomeNewSession 抽出；占位 tab 改选 = 原地重定向，同 id 保草稿，DesktopShell 经 ChatWindow inputLeadingControl 注入）；折叠/展开由 ChatToolbar 的 ☰ 开关承担；「工作区」分区标题（与新建任务左对齐）+ 右侧 ＋（新建工作区/导入目录菜单，body-portal）；工作区节点（缩进一级，**整行 = 展开/折叠开关**——文件夹开/合两种形态；**hover 右侧出 工作区首页🏠/归档🗑 两个快捷按钮**，取代徽章/计数，SessionRow 同款 hover-action 模式；不选中高亮；总览/归档由此入口）→ 会话行直挂（共享 `SessionRow` rounded：Cmd/中键新 tab + 行内归档 + 徽章 + 选中圆角高亮；**每组默认 5 条，超出收进「显示更多（共 N）」**，展开为会话内临时态）+ **组尾暗淡「归档」行**（作用域就地）；不可用工作区暗淡行挂树底；底部四入口 strip（设置/模型/插件/Skills，圆角按钮内缩）。折叠持久化 `pi-tree-collapsed`；骨架门控同旧（未加载渲染骨架行）。会话分组/排序复用 `lib/home-quick-switch.ts` 的 `groupSessionsByWorkspace`（含空工作区的 header-only 组）。知识库/工作项/Loops 不进树（用户低频；住右坞 + 总览 hub，见下）。
**中央区整页（CenterPage，`useAppShellState` 的 `centerPage` 状态）**：底部四入口与树内归档行打开的配置/归档面渲染在**中央区整页**（SessionTabBar 下方）——推广旧设置›工作区页模式：列表+详情并排在同一页面。点任何会话 tab（activateTab）即关闭回聊天；从不持久化；toggle 语义（同页再点关）。设置 = `SettingsPanel` desktop 模式（索引只列 工作区/偏好，子页面板内推进航 ‹ 设置 返回；工作区子页 = inline-split WorkspaceManager 经 workspaceSlot 注入，meta 用 workspaceSettingsName）；模型/Skills/插件 = 三个 Config 组件的 **`inline` 模式**（左 280px 列表列 + 右 banner/详情/页脚——旧三列 portal split（configPortalNode）已整体退役，`split/portalTarget` prop 不复存在）；归档 = ArchiveModal embedded（带工作区名 meta）。Skills/插件面板保持双作用域（全局 + 当前工作区项目层，`settingsCwd`/`globalOnly` 不变）；模型保持全局默认 + per-session 选择器（manifest.agent.default_model 死字段不激活）。移动端设置面板不变（全行索引 + 内嵌子页，不传 desktop prop）。**右坞不变**（2026-09 右坞 S1/S2/S3）：文件/Loops/知识库/工作项 tab 照旧，工作项 = WorkspaceManager panel 模式，知识库 = KnowledgeBrowser，Loops = LoopsDockPanel；总览 hub 不变。
...
The panel bodies（树形侧栏改版后）：左栏只剩 `ProjectSidebar` 一棵树（见上）；配置/归档面全部走中央区整页（CenterPage，见上）。知识库/工作项/Loops 住**右坞**（S1/S2/S3，不变）：工作项 = **右坞工作项 tab（`WorkspaceManager` panel 模式：列表→详情面板内推进航；门控 = work-items capability；总览工作项按钮/新建 → 开右坞 tab；移动端照旧 = overview 栈 work-items 页）**；知识库 = **右坞知识库 tab（`KnowledgeBrowser` 直挂：chips 选 bundle + FileExplorer + open index.md 提示；门控 = knowledge capability，无 tab 不显示；总览知识库行 → 开右坞 tab；移动端照旧 = overview 栈 knowledge 页）**；Loops = **右坞 Loops tab（`LoopsDockPanel`：列表+状态/暂停恢复/停止本轮/「运行」手动起轮 + 配置/新建内联 LoopsConfig 推导航）**；总览 Loops 区块瘦身为摘要行（`onOpenLoopsTab` → 开右坞 tab）；frontmatter 编辑在 LoopsConfig 配置面，见 Loop 章「Loop 配置面」；STATE.md + workspace git log remain the run observability。设置 = `SettingsPanel`（desktop 模式见上；移动端 = 全行索引 + 内嵌子页，归档行仅移动端）。 The remaining `WorkspaceManager` modal is the home create-workspace wizard only.

**Persistence**: tab strip (`pi-session-tabs`), sidebar width (`pi-sidebar-width`), tree collapse (`pi-tree-collapsed`). The old panel keys (`pi-active-view:<wsId>` / `pi-active-panel`) are RETIRED with the middle column — centerPage 从不持久化。`applyUrlToTabs` only understands the URLs `buildTabQuery` emits (overview/chat); legacy `?view=` spellings are gone — stale bookmarks land on the workspace home tab.

- **项目树侧栏（桌面，`ProjectSidebar`）** — **2026-09 树形侧栏改版（取代图标栏 + 中栏工作台双轨）**：
  新建任务按钮（新会话 composer 入口）+ 工作区分区（工作区节点（整行=展开/折叠，hover 出 工作区首页/归档 快捷钮）→
  会话行直挂（默认 5 条 + 显示更多）+ 组尾归档行；不选中高亮；折叠持久化
  `pi-tree-collapsed`；运行/完成活动点）+ 不可用工作区
  暗淡行 + 分区 ＋（新建工作区/导入目录）+ 底部四入口（设置/模型/插件/Skills → 中央区整页）。会话行 = 共享
  `SessionRow`（rounded：选中圆角高亮；Cmd/中键新 tab、行内归档、徽章）；折叠/展开 = ChatToolbar 的 ☰；
  侧栏默认宽 300px（2026-09 两轮反馈：200 太挤 → 260 仍偏小；拖拽范围不变，双击把手重置到 300）。
  全局会话分组/排序复用 `lib/home-quick-switch.ts`；移动端首页 `HomeSessionGroups`（组头点击=进工作区）不受影响。
- **「文件」tab（桌面右栏 / 移动端独立 tab）** — `components/FilesExplorerPanel.tsx`（2026-09 会话/文件分 tab：
  移动端从工作台堆叠段撤出，独立底部 tab 与桌面右栏同体）：toolbar = `[ 文件 | 改动(N) ]` segmented switch
  (`pi-explorer-tab:<wsId>`, git repos only — one key for both surfaces) + ⟳ manual refresh
  ...
  The Loops panel's reveal signal (`loopFilesReveal`) routes here — DesktopShell activates the「文件」tab / MobileShell switches to the
  文件 tab, and the panel switches its segmented back to 文件 on reveal (tree invisible otherwise).
- **工作区（移动端 tab，菜单化）** — **2026-09 菜单化：底部第 2 个 tab「工作区」，落地即 `WorkspaceHomeMenu`
  （不再渲染 WorkspaceOverview 仪表盘）**（PanelHeader 工作区 + meta 工作区名）：唯一常驻按钮「＋ 新建会话」；
  「会话」行（本工作区计数，→ 会话子页 = `WorkspaceSessionList` 全屏，行内删除）；模块组（capability 门控，
  全关则整组只剩 Loops——它文件即声明、常驻）：工作项（活跃数 → WorkspaceManager panel 子页）/
  知识库（bundle 数 → KnowledgeBrowser 子页）/ Loops（运行/暂停状态 meta → `LoopsDockPanel` 子页，与桌面
  右坞 Loops tab 同体，listHeader 接 ‹返回头，配置/新建在其内联导航）；「工作区设置」行 → 设置›工作区。
  **仓库不设菜单行**（浏览 = 底部「文件」tab，管理 = 设置›工作区）。子页走 overview 栈（只存子页，
  null = 菜单；‹返回回菜单；离开 tab/切工作区丢弃）。**全局跨工作区会话列表只住首页**（HomeLanding
  的 HomeSessionGroups；工作区 tab 不再渲染项目树侧栏）。旧的工作台全局列表、会话+文件堆叠分段、
  拖拽分割与折叠持久化均已删除。
...
**Desktop sidebar（项目树）is drag-resizable.** The width is a CSS var (`--pi-sidebar-width`, default `300px`, min 200) set inline on
...
(`pi-sidebar-width`), clamped `[200, 560]`; double-click the handle resets to 200. Mobile keeps a fixed 280px drawer
...
- **工作项工具**新增 `loop` 绑定透传（见上）；外部源同步已下放工作区脚本（见「外部源适配」节）；subagent 一律走内置体系（全局挂载，见下节——kit 轮会话 cwd=workspace 根，profile 自动按 `<cwd>/.pi/agents/` 发现，无需 per-session 注入）。
...
### Subagent (built-in, upstream v0.9.1 port)

**In-process child AgentSessions** owned by the same daemon registry as
interactive sessions and kit rounds (live SSE / steer / abort through the
existing session surface). Ported from upstream v0.9.1 + fixes `9d282da`/`20a2579`;
the community `@henryqw/pi-subagent` package was removed (its `delegate_task`
result panel remains for history rendering). Canonical doc: `docs/subagent.md`.

- **Tools**: `Agent` (profile, prompt, run_in_background, resume, input_files,
  model/thinking/max_turns/inherit_context/isolation overrides),
  `get_subagent_result`, `steer_subagent`. Children strip them again
  (`SUBAGENT_CONTROL_TOOL_NAMES`) — no recursive orchestration.
- **Always global**: `lib/daemon/rpc-manager.ts` attaches
  `createSubagentExtension` to **every** session (same posture the community
  package had); still NOT in `WORKSPACE_EXTENSION_FACTORIES` /
  `ALL_WORKSPACE_CAPABILITIES`.
- **Profiles** (builtin `general-purpose`/`explore`/`plan` < global
  `~/.pi/agent/agents/*.md` < workspace `<cwd>/.agents/agents/*.md` < project
  `<cwd>/.pi/agents/*.md` — FLAT, not the community package's `pi-subagent/`
  subdir; same name wins upward). Frontmatter: `display_name`/`description`/
  `tools`(+`ext:` selectors)/`skills|load_skills`/`extensions|load_extensions`/
  `model`/`thinking`/`max_turns`/`inherit_context`/`run_in_background`/
  `prompt_mode`/`isolation`/`persist_session`/`enabled`; unmanaged keys
  round-trip on save. CRUD in the settings panel `Agents` section
  (`AgentsConfig.tsx`).
- **Settings**: `~/.pi/agent/agents/settings.json` — `builtInEnabled`
  (fork default ON — replaces the always-on community package; damaged file
  fails OPEN) + `maxConcurrent` (per-parent FIFO, default 10, max 32).
- **Timeouts — upstream semantics**: NO idle kill, NO runtime cap; opt-in
  `max_turns` soft limit only. The heartbeat monitor EXEMPTS subagent children
  (killing a hidden silent child was the community kill policy this fork
  rejected); the settled-idle 10-min teardown is ordinary registry cleanup.
- **Child sessions**: in-process, persisted by default, keep the
  `pi-subagent-<uuid>` id prefix (via `NewSessionOptions.id`) so
  `lib/subagent-child.ts` tagging, sidebar hiding and locate/open work
  unchanged across eras. Run status/status transitions are custom entries in
  the child file (`pi-web:subagent[-status|-result]`) — crash recovery derives
  `interrupted`.
- **Result shape** (MessageView): `details = {kind: "pi-web-subagent",
  sessionId, profile, status, error?, worktreePath?}` with an open-session
  button; `lib/session-subagents.ts` derives the drawer from BOTH transports.
- **Model routing**: per-profile `model`, else inherit the parent's. The old
  `pi-task-models.json` `pi-subagent/delegateTask` key is inert.
...
- APIs: `GET /api/git/status?cwd=`, `GET /api/git/diff?cwd=&path=` — guarded by the file-access allow-list. Polled by
  ...
  `[ 文件 | 改动(N) ]` segmented switch in `FilesExplorerPanel` — ONE component, two hosts (desktop right panel's pinned「文件」tab /
  mobile「文件」tab; both persist the choice in `localStorage` key `pi-explorer-tab:<wsId>` so the surfaces agree) — both
  follow the Explorer's current cwd scope; `ChangesPanel` itself is unchanged.
...
- **dev Loop instance**: `/Users/qiancheng/Documents/Workspace/cxin/`（2026-09 由 pi 托管的 workspace-c
  ...
  双开 guard / `loop.active_session` 戳删除）, roles in `.pi/agents/` (built-in discovery: flat `*.md` profiles — see Subagent below). v3 leftovers (`loop.yaml`, `RUNS.jsonl`,
...
~/.pi/
  ...
  workspaces/                            ($PI_WORKSPACES_DIR; 2026-09 起工作区也可以是任意外部路径)
    workspace-<slug>/                    （或任意外部路径的工作区根）
      ...
      .pi/agents/*.md                    subagent 项目 profile（平铺）；.pi/cache/ 可重建缓存
...
lib/
  ...
  daemon/                           THE pi-daemon: session-owning process + job registry (C2)
    host.ts                 createDaemon() composition root — /health, route chain (sessions), jobs (loop-kit-heartbeats), startDaemon (PI_DAEMON_HOST/PORT)
    ...
    client.ts               daemonClient — HTTP client to the daemon (PI_DAEMON_URL): session-daemon surface
    rpc-manager.ts          DAEMON-ONLY session registry + AgentSessionWrapper + startRpcSession (moved here from lib/ root — the daemon domain's core; attaches the global built-in subagent extension + wires SUBAGENT_CONTROLLER)
  ...
  subagent-child.ts          subagentChild tagging by `pi-subagent-` id/name prefix (both eras: builtin children keep the prefix)
  ...
  session-tabs.ts           会话 tab 模型纯逻辑层（+测试）：SessionTabState（session/new-session/workspace-home 三种 kind，id 前缀 s:/new:/ws:）、createX 工厂、resolveOpenSessionTarget（C1 分派）、nextActiveTabId（X1 邻居回落）、contextCloseTargetIds/nextActiveAfterBatchClose（右键批量关闭：其他/左侧/右侧目标集合 + 批量版 X1 激活回落）、tabQuery（URL 投影）、serializeTabs/parseStoredTabs/restoreTabs（R2 持久化，失效 id 静默跳过）
...
components/
  ...
  shell/
    ...
    DesktopShell.tsx        the desktop render（2026-09 树形侧栏改版）：ProjectSidebar（可拖宽/折叠，复用 sidebar-container 类）+ center（ChatToolbar + SessionTabBar + CenterPage/总览/聊天/首页 composer）+ right dock；中央区整页渲染分派在本组件（renderCenterPage）
    MobileShell.tsx         the mobile shell: ChatToolbar + SessionTabBar (chips row, hidden on the home landing) + tab-switched main area (会话/工作区/文件/设置, `TAB_ORDER`; 工作区 tab 落地 = WorkspaceHomeMenu，子页栈 sessions/work-items/knowledge/loops；文件 = FilesExplorerPanel) + bottom tab bar (narrow) / left `MobileSideRail` (≥768px wide viewports, same tab state) + per-tab secondary stacks (archive) + full-screen file overlay + persistent chat mount
  ...
  HomeSessionGroups.tsx    全局会话分组列表（按工作区分组：组头点击=打开工作区，chevron 折叠默认全展开不持久化；会话行=共享 SessionRow）。宿主（树形侧栏改版后）：仅移动端 HomeLanding 最近区（过滤空组，不传管理 props）；桌面的会话树在 ProjectSidebar 内直构（复用 groupSessionsByWorkspace + SessionRow，不用本组件）；排序由 lib/home-quick-switch.ts 的 groupSessionsByWorkspace 负责（活跃度降序、空组沉底）
  SessionRow.tsx            会话行共享组件（运行/完成徽章 + Cmd/中键新 tab C1 手势 + 行内归档 + 可选相对时间；hover 只留归档按钮）——HomeSessionGroups 与移动端首页最近区共用
  ...
  ProjectSidebar.tsx         项目树侧栏（桌面左侧单一侧栏，2026-09 改版）：新建任务按钮（折叠/展开 = ChatToolbar ☰）+ 工作区分区（工作区节点整行=展开/折叠（hover 出 工作区首页/归档 钮）→会话行直挂（默认 5 条 + 显示更多，SessionRow rounded）+ 组尾归档行；折叠持久化 pi-tree-collapsed）+ 不可用工作区暗淡行 + 分区 ＋（新建/导入工作区）+ 底部四入口 strip（设置/模型/插件/Skills → CenterPage）；会话分组复用 lib/home-quick-switch；骨架门控；图标栏 ActivityBar 与中栏面板 WorkspaceSidebar 已随改版删除（移动端 tab 图标已内联进 MobileShell）
  ...
  FilesExplorerPanel.tsx   the「文件」tab body（桌面右栏钉死 tab + 移动端独立底部 tab 同体）— [ 文件 | 改动(N) ] toolbar + ⟳ + FileExplorer/ChangesPanel (cwd = workspace root); exports ExplorerSegmentedTabs; reveal switches its segmented back to 文件
  ...
  WorkspaceOverview.tsx     workspace dashboard — the DESKTOP 家 tab landing (2026-09 菜单化后移动端不再渲染，改用 WorkspaceHomeMenu)：quick actions, 活跃工作项 with 「立即跑一轮」B-button + Loop 绑定下拉, 仓库/知识库 rows, 「会话」区块（WorkspaceSessionList），Loops 摘要行（onOpenLoopsTab → 右坞 Loops tab，必传）
  WorkspaceHomeMenu.tsx     移动端工作区 tab 落地菜单（2026-09 菜单化）：＋新建会话 + 会话（计数）+ 模块组（工作项/知识库 capability 门控、Loops 常驻带状态 meta）+ 工作区设置；计数自取（work-items 活跃数/repositories 知识库数/loops/会话数），仓库不设行（浏览=文件 tab，管理=设置）
  WorkspaceSessionList.tsx  本工作区会话列表（拉 /api/sessions 后前缀过滤、排除 subagent 子会话）：默认前 20 条 + 显示全部，行内删除（RecentSessionRow）；宿主 = 移动端工作区 tab「会话」子页（showHeader=false）+ WorkspaceOverview「会话」区块（桌面家 tab）
  SessionTabBar.tsx          the SESSION tab strip（2026-09 会话 tab 化，替换已删的 WorkspaceTabBar）：首页固定首 tab + 会话/占位/家 tab chips（运行呼吸点/未看完成点 + 拖拽排序）+ ＋新会话（无活动 tab 时隐藏）+ ⊞工作区选择器（可选：桌面已退役——项目树常驻接管工作区导航；移动端保留，body-portal dropdown → 开/激活家 tab）+ tab 右键菜单（关闭/关闭其他/关闭左侧/关闭右侧——目标集合 contextCloseTargetIds，批量关闭走 useAppShellState.closeTabs：合并草稿确认只弹一次、运行中 tab 照常摘除不终止会话；菜单定位复用 computeMenuLayout + portal 遮罩，Escape/滚轮/拖拽关闭）；两 shell 共用，无 isMobile 分支
  ...
  LoopsDockPanel.tsx        右坞 Loops tab 本体（右坞 S1）+ 移动端工作区 tab「Loops」子页（2026-09 菜单化，listHeader 接 ‹返回头）：loop 列表+状态/操作 + 内联 LoopsConfig 推导航；key=workspace.id 挂载（同工作区切 tab 状态存活）
...
The built-in subagent tools (`Agent` / `get_subagent_result` / `steer_subagent`) are attached to **every** session in `rpc-manager.ts` via `createSubagentExtension` (`lib/subagent-extension.ts` + `SUBAGENT_CONTROLLER` wiring in the same file), regardless of workspace or capability. They are intentionally absent from `WORKSPACE_EXTENSION_FACTORIES` and `ALL_WORKSPACE_CAPABILITIES`. Profile discovery is cwd-based (`listSubagentProfiles(cwd)` reads `<cwd>/.pi/agents/` flat) and loop sessions run with cwd = workspace root, so workspace profiles load with zero wiring. Timeouts follow upstream semantics — no idle kill, no runtime cap; the daemon heartbeat monitor exempts subagent children (see the Subagent section).
...
`entryIds[]` in `SessionContext` is a parallel array to `messages[]` — it maps each displayed message back to its `.jsonl` entry id, used for fork and `navigate_tree` calls. Subagent children (`pi-subagent-<uuid>` ids — builtin children are created with this prefix; the legacy community-package children used it too) use the same format; the sidebar hides them via the id/name prefix check in `lib/subagent-child.ts`. Builtin children additionally carry a `parentSession` link + `pi-web:subagent` meta entries (crash recovery + resume read them).
---
