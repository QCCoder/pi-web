...
- **项目树侧栏（桌面，`ProjectSidebar`）** — **2026-09 树形侧栏改版（取代图标栏 + 中栏工作台双轨）**：
  ...
  侧栏默认宽 = 最小宽 200px（拖拽范围不变，双击把手重置到 200）。
...
**Desktop sidebar（项目树）is drag-resizable.** The width is a CSS var (`--pi-sidebar-width`, default `200px` = the min) set inline on
