...
- **Workspace management**: `app/api/workspaces/**` reads/writes `~/.pi/workspaces/**` and `~/.pi/workspace.yaml` (the global index). Capability edits, repository add/remove, work-item CRUD) all flow through this surface. Kit loops are plain files under the workspace root — there is no loop management surface.
...
- **Attachment point**: `buildWorkspaceExtensions(manifest, path)` filters factories by effective capabilities. `lib/daemon/rpc-manager.ts` always attaches `createSubagentExtension(...)` globally, then — when the session's cwd is inside a workspace — appends `buildWorkspaceExtensions(...)` and filters skills to `manifest.skills`.
...
**Module group (workspace-scoped, capability-gated): 工作台(workbench) → 知识库(knowledge) → 工作项(work-items)** — `workbench` is always shown (`capability: null`); then a **separator + global group (app-scoped): 模型(models) → Skills(skills) → 插件(plugins) → 归档（needs an active workspace）→ 设置（pinned to the rail bottom）**.
...
- **心跳**：daemon 的 `LoopKitSpawner`（DaemonJob `loop-kit-heartbeats`，30s tick，分钟槽去重；**无条件注册，无旗子**）
  扫已注册 workspace，到点 → `startRpcSession` 起一次性会话（cwd=workspace 根 → workspace 装配 + extraAgentDirs
  重推导照常），命名 `<loop> · <slot>`，开场合同 = LOOP.md 正文 + spawner 注入的硬规则 + `/skill:<pattern>` 展开；
...
- **run-contract**：工作项「开始对话/收养续跑」= 客户端预填（D11，无 daemon seed 路由）——`handleRunContract` in
  ...
  (ChatWindow keys its ChatInput mount on it so an already-mounted composer re-reads the draft) and switches to the
...
- **工作项工具 / importer / 内置 subagent 不变**（D10：社区 subagent 包仅 GitHub Actions 场景，经 `.pi/settings.json` 声明）。
...
- **Always global**: `lib/daemon/rpc-manager.ts` attaches `createSubagentExtension(...)` to **every** session regardless of
...
- **API**: none — the only loop-related pi-web route is the read-only `GET /api/workspaces/[id]/loops`.
...
- **dev Loop instance**: `~/.pi/workspaces/workspace-c/` in **kit form** (kit-rehearsal branch merged): `loops/dev-loop/LOOP.md`
  ...
  双开 guard / `loop.active_session` 戳删除）, five roles in `.pi/agents/`. v3 leftovers (`loop.yaml`, `RUNS.jsonl`,
...
lib/
  abort-race.ts             raceAbort (promise vs AbortSignal, with onLateSettle cleanup) + creationTimeoutSignal — bounds session CREATION everywhere a startRpcSession hang would pin a turn/run (subagent spawn, kit round)
  workspaces/
  ...
  subagent/
    ...
    agents.ts               discover agents (user / project / injected extraAgentDirs), built-in "general"
...
components/
  ...
  shell/
    ...
    MobileShell.tsx         the mobile shell: ChatToolbar + WorkspaceTabBar + bottom-tab-switched main area (会话/工作台/知识库/工作项/设置, `TAB_ORDER`/`TAB_CAPABILITY`) + per-tab secondary stacks (archive) + full-screen file overlay + persistent chat mount
  ...
  WorkspaceOverview.tsx     workspace dashboard — the unconditional landing view (quick actions, 活跃工作项, 仓库/知识库 rows that navigate the sidebar, recent sessions with delete)
...
**Watching a live session (any session — interactive, subagent child, or kit loop round) is one mechanism now (C2).** The daemon owns every session; `/api/agent/[id]/events` is a pure pipe onto `/v1/sessions/:id/events`, which resolves via `findLiveSession` (the ordinary registry, where subagent children and kit rounds live alongside interactive sessions) and cold-starts idle sessions for viewing. Two subtleties remain:
...
`entryIds[]` in `SessionContext` is a parallel array to `messages[]` — it maps each displayed message back to its `.jsonl` entry id, used for fork and `navigate_tree` calls. Subagent child sessions use the same format and link back via `parentSession`; their ids are recorded in `~/.pi/agent/subagent-children.txt` so the sidebar hides them.
