...
- **工作项绑定 + 管理面**（协议语义落文：`kit/README.md` 工作项绑定一节）：工作项可声明 `loop: <loop名>` 绑定（S1 软校验，
  ...
  cron 改后下一心跳自然生效，`.lastrun` 保留、next-due 按新 cron 重算）。④ Overview Loops 区块（有 loop 才渲染）：
