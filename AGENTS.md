...
lib/
  ...
  tab-types.ts              Tab 类型（file/session 文件 tab）+ FILES_TAB_ID——从 TabBar.tsx 下沉到此供 lib 层引用
...
components/
  ...
  WorkspaceManager.tsx      workspace create/import + settings (`embedded` in-panel, `panel` narrow single-column variant, `split` has two variants: `{ portalTarget }` = rail inline + detail portaled (the 工作项 panel pattern), `{ inline: true }` = 列表+详情并排单容器（设置›工作区的中央区页面，2026-09 搬家）; the settings detail carries the 基本信息 card — rename (PATCH name) + 暂时停用 toggle (PATCH disabled; disabled workspaces keep a settings-only「已停用」badge, the sole re-enable surface)）; remaining modal = home create-workspace wizard
  ...
  LoopRow.tsx               loop 单行（name/cron 摘要/级别/状态 + 配置/暂停/停止/可选运行）——总览 Loops 区块专用（LoopsPanel 已随 W-中退役，「运行」按钮由总览 onRunLoop 接管）
  KnowledgeBrowser.tsx      知识库浏览（原中栏知识库模块视图的提升版：chips 选 bundle + FileExplorer；挂在家 tab hub 的 knowledge 子视图 / 移动 overview 栈页）
