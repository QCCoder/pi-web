...
- **项目树侧栏（桌面，`ProjectSidebar`）** — **2026-09 树形侧栏改版（取代图标栏 + 中栏工作台双轨）**：
  新建任务按钮（新会话 composer 入口）+ 工作区分区（工作区节点（整行点击=展开/折叠，文件夹开/合两态）→
...
components/
  ...
  ProjectSidebar.tsx         项目树侧栏（桌面左侧单一侧栏，2026-09 改版）：新建任务按钮（折叠/展开 = ChatToolbar ☰）+ 工作区分区（工作区节点整行点击=展开/折叠（文件夹开/合两态）→会话行直挂（默认 5 条 + 显示更多，SessionRow rounded）+ 组尾归档行；折叠持久化 pi-tree-collapsed）+ 不可用工作区暗淡行 + 分区 ＋（新建/导入工作区）+ 底部四入口 strip（设置/模型/插件/Skills → CenterPage）；会话分组复用 lib/home-quick-switch；骨架门控；图标栏 ActivityBar 与中栏面板 WorkspaceSidebar 已随改版删除（移动端 tab 图标已内联进 MobileShell）
