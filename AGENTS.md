...
components/
  ...
  WorkspaceOverview.tsx     workspace dashboard — the unconditional landing view (quick actions, 活跃工作项 with 「立即跑一轮」B-button + Loop 绑定下拉, 仓库/知识库 rows that navigate the sidebar, recent sessions with delete, and a Loops 区块: 桌面 = 摘要行（onOpenLoopsTab → 右坞 Loops tab，决策 #10），移动端 = 全量管理区块（LoopRow 列表+配置/新建 → overview 栈）)
