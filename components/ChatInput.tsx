...
export function getUserMessageText(message: UserMessage): string {
  const text = typeof message.content === "string"
    ...
    : message.content
        ...
        .join("\n");
  // Local delta vs upstream: a pi skill-wrapper message restores as its
  // /skill:name command form, matching what the message bubble displays.
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  // Cap the @ file menu to the visible space above the composer so a long
  // match list cannot paint through the session tab bar; the list scrolls
  // inside the cap and remeasures when the surrounding layout shifts.
  useLayoutEffect(() => {
    ...
    (e: KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (e.key === "Enter" && !e.shiftKey && !isMobile) {
        ...
        if (isStreaming && (onSteer || onFollowUp)) {
          ...
          // follow-up so a new turn can be lined up mid-stream (upstream #657).
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    // Rich-text sources (browsers, Office, etc.) carry links only in the
    // text/html flavor; the plain-text flavor flattens them to their labels.
    // Rebuild Markdown links from the HTML so pasted URLs survive.
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", ...(isMobile ? { display: "flex", flexDirection: "column" } : null) }}>
        ...
        {/* Main input */}
        <div style={{ position: "relative", minWidth: 0, ...(isMobile ? { order: 2 } : null) }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, alignSelf: "flex-end" }}>
              ...
              {onFollowUp && (
                <button
                  ...
                  disabled={!canQueueStreamingMessage}
                  title={`${attachedImages.length ? "Image attachments cannot be queued while the agent is running" : "Queue this message after the agent finishes"}${isMobile ? "" : " (Alt/Option+Enter)"}`}
                  aria-keyshortcuts={isMobile ? undefined : "Alt+Enter"}
                  style={{
                    display: "flex", alignItems: "center", gap: 5,
                    padding: "7px 12px",
